...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.get('is_registered'):
        ...
        # Ask language first for new users
        ...
        # state will proceed to awaiting_ign after lang selection
...
async def cmd_tournaments(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for tk in tourneys:
        ...
        if is_in:
            ...
            kb_rows.append([InlineKeyboardButton("⚔️ Join", callback_data=f"t_join_{tk['id']}")])
        else:
